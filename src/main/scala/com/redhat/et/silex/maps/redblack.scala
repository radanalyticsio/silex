...
package com.redhat.et.silex.maps.redblack
...
object tree {
  ...
  trait Node[K] {
    ...
    final protected
    def rNode(d: Data[K], lsub: Node[K], rsub: Node[K]) = iNode(R, d, lsub, rsub)

    /** create a new Black node from a key, value, left subtree and right subtree */
    final protected
    def bNode(d: Data[K], lsub: Node[K], rsub: Node[K]) = iNode(B, d, lsub, rsub)
    ...
    final protected
    def balanceDel(x: Data[K], tl: Node[K], tr: Node[K]) =
      (tl, tr) match {
    ...
    final protected
    def balanceLeft(x: Data[K], tl: Node[K], tr: Node[K]) =
      (tl, tr) match {
    ...
    final protected
    def balanceRight(x: Data[K], tl: Node[K], tr: Node[K]) =
      (tl, tr) match {
    ...
    final protected
    def append(tl: Node[K], tr: Node[K]): Node[K] = (tl, tr) match {
      case (LNode(), n) => n
      case (n, LNode()) => n
